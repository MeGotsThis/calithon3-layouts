class GdqRunEditor extends Polymer.Element {
  ...
  applyChanges() {
    ...
    const runners = [];
    const runnerNameInputs =
      Polymer.dom(this.$.runners)
      .querySelectorAll('paper-input[label^="Runner"]:not([disabled])');
    const runnerStreamInputs =
      Polymer.dom(this.$.runners)
      .querySelectorAll('paper-input[label="Twitch Channel"]:not([disabled])');
    for (let i = 0; i < 4; i++) {
