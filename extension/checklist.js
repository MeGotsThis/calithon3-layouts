...
// To edit the list of checklist items, edit the "default" value of
// schemas/checklist.json.
// Any changes you make will be fully picked up and integrated next time NodeCG
// starts.
...
// Reconcile differences between persisted value and what we expect the
// checklistDefault to be.
  ...
  for (const category in checklistDefault) {
    ...
    mergedChecklist[category] = checklistDefault[category].map((task) => {
      if (persistedValue[category]) {
        const persistedTask =
          persistedValue[category].find(({name}) => name === task.name);
        if (persistedTask) {
