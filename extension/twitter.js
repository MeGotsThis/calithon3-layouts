...
setInterval(() => {
  nodecg.log.info(
    '[twitter] Restarting Twitter connection (done every 90 minutes).');
  userStream.close();
  buildUserStream();
}, 90 * 60 * 1000);
...
/**
 * Builds the stream. Called once every 90 minutes because sometimes the stream
 * just dies silently.
 * @return {undefined}
 */
...
function addTweet(tweet) {
  ...
  tweet.text = tweet.text.replace(
    /#sgdq2017/ig, '<span class="hashtag">#SGDQ2017</span>');
 ...
 * @return {Object} - The removed tweet. "Undefined" if tweet not found.
 */
function removeTweetById(idToRemove) {
  if (typeof idToRemove !== 'string') {
    throw new Error(
      `[twitter] Must provide a string ID when removing a tweet. ID provided `
      + `was: ${idToRemove}`);
  }
