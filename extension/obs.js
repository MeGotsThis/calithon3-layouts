...
if (uploadScriptPath) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      throw new Error(
        `Configured youtubeUploadScriptPath (${uploadScriptPath}) does not '
        + 'exist.`);
    } else {
      throw e;
    }
  ...
  if (!stats.isFile()) {
    throw new Error(
      `Configured youtubeUploadScriptPath (${uploadScriptPath}) is not a `
      + `file.`);
  }
...
autoCycleRecordings.on('change', (newVal) => {
  nodecg.log.info(
    'Automatic cycling of recordings %s.', newVal ? 'ENABLED' : 'DISABLED');
  if (!newVal) {
...
autoUploadRecordings.on('change', (newVal) => {
  nodecg.log.info(
    'Automatic uploading of recordings %s.', newVal ? 'ENABLED' : 'DISABLED');
});
...
streamingOBS.replicants.programScene.on('change', (newVal) => {
  ...
  newVal.sources.some((source) => {
    ...
    if (lowercaseSourceName.indexOf('layout') === 0) {
      currentLayout.value =
        lowercaseSourceName.replace(/ /g, '_').replace('layout_', '');
      return true;
    }
...
function cycleRecording(obs) {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(
        new Error(`Timed out waiting for ${obs.namespace} to stop recording.`));
    }, 10000);
...
module.exports = {
  ...
  async cycleRecordings() {
    ...
    if (recordingOBS._connected) {
      ...
      nodecg.log.error(
        'Recording OBS is disconnected! Not cycling its recording.');
    }
    ...
    if (streamingOBS._connected) {
      ...
      nodecg.log.error(
        'Streaming OBS is disconnected! Not cycling its recording.');
    }

    if (cycleRecordingPromises.length <= 0) {
      nodecg.log.warn(
        'Neither instance of OBS is connected, aborting cycleRecordings.');
      return;
    }
