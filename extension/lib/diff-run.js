 ...
 * @return {Object|undefined} - The original values of any modified properties.
 */
function calcOriginalValues(run, original) {
  ...
  differences.forEach((difference) => {
    switch (difference.kind) {
      case 'N':
        // The only place that 'N' differences can happen is in the "runners"
        // array.
        ...
        break;
      case 'D':
        // The only place that 'D' differences can happen is in the "runners"
        // array.
        /* istanbul ignore else: shouldn't be possible to enter the else path */
        if (difference.path[0] === 'runners') {
          if (!originalValues.runners) {
        ...
        break;
      case 'A':
        // The only place that 'A' differences can happen is in the "runners"
        // array.
        /* istanbul ignore else: shouldn't be possible to enter the else path */
...
/**
 * Given an active run (currentRun or nextRun) and that same unmodified
 * (but formatted) from the schedule, returns a new run object with new changes
 * from the tracker incorporated.
 * @param {Object} run - An active run (currentRun or nextRun)
 * @param {Object} unmodifiedRun - An unmodified (but formatted) run from the
 * schedule.
 * @return {Object} - The merged run.
 */
function mergeChangesFromTracker(run, unmodifiedRun) {
  // Immediately clone everything, because we can return either and the diffing
  // code relies on currentRun and nextRun never being the same object as what
  // is in the schedule replicant.
  run = clone(run);
