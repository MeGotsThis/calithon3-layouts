 ...
 * @param {string} startTime - A formatted time string, such as 1:00 for one
 * hour.
 * @return {undefined}
 */
function start(startTime) {
  ...
  const timeStruct =
    TimeUtils.createTimeStruct(TimeUtils.parseTimeString(startTime));
  if (timeStruct.raw <= 0) {
    return;
  }
...
/**
 * Ticks the countdown timer down by one second, stopping the timer if it hits
 * zero.
 * @return {undefined}
 */
function tick() {
