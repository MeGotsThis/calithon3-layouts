(function() {
  ...
  /**
   * The main bids and prizes loop. Only call it once, it will call itself after
   * that.
   * @return {undefined}
   */
  function loop() {
    ...
    switch (numToShow) {
      ...
      default: {
        ...
        // Then, check if there are any bids or prizes to show once every
        // second.
  ...
  /**
   * Changes the text content of the header above the main screen, where bids
   * and prizes are shown. Fades the text out, changes it, then fades it back
   * in.
   * @param {string} newText - The text to put in the header.
   * @return {TweenLite} - A TweenLite animation.
   */
   ...
   * @return {Promise} - A promise that resolves as soon as the static has
   * finished entering, so that you can do work while the static hides the jank.
   * The static will begin exiting STATIC_HOLD_DURATION milliseconds after the
   * promise is resolved, and the exit itself takes STATIC_FADE_DURATION
   * milliseconds.
   */
