...
class GdqSteppedGradient extends SteppedGradientMixin(Polymer.Element) {
  ...
  enter(aboveOrBelow) {
    ...
    tl.to({}, 0.01, {
      onComplete() {
        let zIndex = aboveOrBelow === 'above' ? 'frontToBack' : 'backToFront';
        this.zIndexing = zIndex;
    ...
    let bands = aboveOrBelow === 'above' ?
      this.bands.slice(0).reverse() :
      this.bands;
    tl.staggerTo(bands, 0.334, {
      y: '0%',
      ease: Sine.easeInOut,
    }, 0.1167);
  ...
  exit(aboveOrBelow) {
    ...
    tl.to({}, 0.01, {
      onComplete() {
        this.zIndexing = aboveOrBelow === 'above' ?
          'frontToBack' :
          'backToFront';
    ...
    tl.add('bandsExit');
    const bands = aboveOrBelow === 'above' ?
      this.bands :
      this.bands.slice(0).reverse();
