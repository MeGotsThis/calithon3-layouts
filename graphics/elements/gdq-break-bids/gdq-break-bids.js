...
(function() {
  ...
  class GdqBreakBids extends Polymer.Element {
     ...
     * @param {Array} bidsArray - The parent array containing all the bids
     * being shown in this cycle.
     * @return {undefined}
     */
    showBid(bid, index, bidsArray) {
      ...
      this.tl.to({}, 0.03, {
        onComplete() {
          ...
          this.$['challenge-leftarrow'].src =
            `${this.importPath}img/${this.bitsChallenge ? 'bits_' : ''}`
            + `challenge_leftarrow.png`;
          this.$['challenge-rightarrow'].src =
            `${this.importPath}img/${this.bitsChallenge ? 'bits_' : ''}`
            + `challenge_rightarrow.png`;
        },
        callbackScope: this,
      });
      ...
      if (bid.type === 'challenge') {
        ...
        this.tl.call(() => {
          if (bid.isBitsChallenge) {
            this.$['challenge-goal'].innerHTML =
              `<img id="challenge-goal-bitsIcon" width="44" `
              + `src="img/bitsicon.png">${bid.goal.replace('$', '')}`;
            this.$['challenge-bar-fill-label-text'].innerHTML =
              '<img id="challenge-bar-fill-label-text-bitsIcon" width="26" '
              + 'src="img/bitsicon.png">0';
          } else {
            this.$['challenge-goal'].innerHTML = bid.goal;
            this.$['challenge-bar-fill-label-text'].innerHTML = '$0';
          }
        });
        ...
        this.tl.call(() => {
          ...
          this.shadowRoot
            .querySelector('#tug-left .tug-option-total').innerHTML =
              bid.options[0].total;
          this.shadowRoot
            .querySelector('#tug-right .tug-option-total').innerHTML =
              bid.options[1].total;
          this.shadowRoot
            .querySelector('#tug-left .tug-option-desc').innerHTML =
              bid.options[0].description || bid.options[0].name;
          this.shadowRoot
            .querySelector('#tug-right .tug-option-desc').innerHTML =
              bid.options[1].description || bid.options[1].name;
        ...
        this.tl.call(() => {
          ...
          qsa('.choice-row-label').forEach((el, index) => {
            el.textContent =
              bid.options[index].description || bid.options[index].name;
          });
      ...
      if (index === 0) {
        ...
        const previousBidDescriptionHeight =
          bidsArray[index - 1].type === 'challenge' ? 80 : 45;
      ...
      switch (bid.type) {
        case 'choice-binary': {
          ...
          let rightPips =
            Math.floor(maxPips * (bid.options[1].rawTotal / bid.rawTotal));
          rightPips = Math.min(rightPips, maxPips);
          const barDeltaDuration =
            Math.abs(leftPips - rightPips) * TUG_PIP_INTERVAL;

          // Only the left bar needs to be animated, right bar just takes
          // remaining space.
          ...
          this.tl.to(deltaTweenProxy, barDeltaDuration, {
            onStart() {
              if (bid.options[0].rawTotal > bid.options[1].rawTotal) {
                ...
                this.$['tug-bar-center-label-leftarrow'].style.display =
                  'block';
              } else if (bid.options[0].rawTotal < bid.options[1].rawTotal) {
                this.$['tug-bar-center-label-rightarrow'].style.display =
                  'block';
        ...
        case 'choice-many': {
          ...
          rows.forEach((row, index) => {
            ...
            let numPips = Math.floor(
              maxPips * (option.rawTotal / bid.options[0].rawTotal));
            ...
            if (index === 0) {
              ...
              barFills = `barFills+=${CHOICE_PIP_INTERVAL * 2 * index}`;
            }
            this.tl.to(
              row.querySelector('.choice-row-meter-fill'),
              barFillDuration, {
          ...
          this.tl.call(() => {
            ...
            if (diff > 0) {
              ...
              this.tl.pause();
              TweenLite.to(
                this.shadowRoot.querySelectorAll('.choice-row'),
                diff * timePerPixel, {
                  y: -diff,
                  ease: Linear.easeNone,
                  delay: 1.5,
        ...
        case 'challenge': {
          ...
          this.tl.to(rawTotalTweenProxy, barFillDuration, {
            ...
            onUpdate() {
              ...
              if (bid.isBitsChallenge) {
                this.$['challenge-bar-fill-label-text'].innerHTML =
                  `<img id="challenge-bar-fill-label-text-bitsIcon" `
                  + `width="26" src="img/bitsicon.png">${formattedTotal}`;
              } else {
                this.$['challenge-bar-fill-label-text'].innerHTML =
                  `$${formattedTotal}`;
              }
    ...
    _calcBidsToDisplay(bidsArray) {
      ...
      bidsArray.forEach((bid) => {
        ...
        // We cant handle choice-many bids that have less than 3 options, which
        // is totally valid but we didn't think about. Just don't show them.
        ...
        // third bid.
        // This ensures that we are never displaying a partial list of bids
        // for a given speedrun.
        const idx = bidsToDisplay.length - 1;
        if (bidsToDisplay.length < 3) {
          bidsToDisplay.push(bid);
        } else if (bid.speedrun === bidsToDisplay[idx].speedrun) {
          bidsToDisplay.push(bid);
        }
      });
