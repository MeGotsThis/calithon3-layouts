  ...
  class GdqOmnibar extends Polymer.Element {
        ...
        replicant.once('change', () => {
          ...
          if (numDeclared >= replicants.length) {
            ...
            total.on('change', (newVal, oldVal) => {
              if (oldVal && newVal.raw >= AGDQ17_TOTAL
                  && oldVal.raw < AGDQ17_TOTAL) {
                this.alertNewRecord();
              }
            });
          }
    ...
    showMainContent(tl, elements) {
      ...
      tl.call(() => {
        ...
        tl.shiftChildren(contentEnterAnim.duration(), true,
    ...
    hideMainContent(tl, elements) {
      ...
      tl.call(() => {
        ...
        tl.shiftChildren(contentExitAnim.duration(), true,
    ...
    showUpNext() {
      ...
      if (currentLayout.value === 'break'
          || currentLayout.value === 'interview') {
    ...
    showChallenges() {
      ...
      currentBids.value.forEach((bid) => {
        ...
        let idx = bidsToDisplay.length - 1;
    ...
    showChoices() {
      ...
      bidsToDisplay.forEach((bid, index) => {
        ...
        const elements = bid.options.slice(0, 4).map((option, index) => {
          ...
          // Options that aren't the first option show their delta to the
          // leader options.
          if (index > 0 && bid.options[0].rawTotal > option.rawTotal) {
            let diff = bid.options[0].rawTotal - option.rawTotal;
            element.delta = '-$' + diff.toLocaleString('en-US');
          }
        ...
        let description = bid.description.replace('||', ' -- ');
        if (index === 0) {
          tl.add(this.showLabel(description, '16px', {
            startColor: '#ae7fc1',
            endColor: '#71338a',
          }), '+=0.03');
        } else {
          tl.add(this.changeLabelText(description), '+=0.03');
        }
