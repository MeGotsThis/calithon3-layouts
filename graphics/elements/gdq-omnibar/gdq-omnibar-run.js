class GdqOmnibarRun extends Polymer.Element {
  ...
  concatenateRunners(run) {
    if (run.pk === 2274) {
      ...
      return 'aperturegrillz, Poochy.EXE, KevinDDR, EnchantressOfNumbers, '
        + 'JBroms, MxKai3, PARTY MAN X & eihoppe';
    } else if (run.pk === 2255) {
      // Super Mario Series Warpless Relay Race Warpless
      return 'Just_defend, bjw, Svenne, darbian, Dotsarecool, xsvArea51, '
        + 'truman, LackAttack24, grandpoobear, Kosmicd12, MrCab, Kirua, '
        + 'SuperSonic71087, Aweglib & RaikouRider';
    }

    let concatenatedRunners = run.runners[0].name;
    if (run.runners.length > 1) {
      let reducer = (prev, curr, index, array) => {
      ...
      concatenatedRunners = run.runners.slice(1)
        .reduce(reducer, concatenatedRunners);
    }
