(function() {
  ...
  class GdqInterviewMonitor extends Polymer.MutableData(Polymer.Element) {
    ...
    onScreenTweetChanged(newVal, oldVal) {
      ...
      firstMonitorTweet.$.material.style.transition =
        'background-color 1600ms cubic-bezier(0.455, 0.03, 0.515, 0.955)';
    ...
    updateUpNextDisplay() {
      ...
      if (upNextRun.runners.length === 1) {
        ...
        concatenatedRunners =
          upNextRun.runners.slice(1).reduce((prev, curr, index, array) => {
    ...
    totalChanged(newVal) {
      ...
      TweenLite.to(this.$['total-amount'], duration, {
        ...
        ease: Power2.easeOut,
        onUpdate() {
          this.$['total-amount'].textContent =
            this.$['total-amount'].rawValue.toLocaleString('en-US', {
