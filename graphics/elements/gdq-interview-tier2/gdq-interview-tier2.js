(function() {
  ...
  const lowerthirdTimeRemaining = nodecg.Replicant(
    'interview:lowerthirdTimeRemaining');
  const questionTimeRemaining = nodecg.Replicant(
    'interview:questionTimeRemaining');

  class GdqInterviewTier2
      extends Polymer.MutableData(
    ...
    ready() {
      ...
      this.$.list.createMirror = (originalElement) => {
        ...
        mirror.querySelector('gdq-tweet').tweet =
          originalElement.querySelector('gdq-tweet').tweet;
        return mirror;
      };
      ...
      interviewNames.on('change', (newVal) => {
        const typeaheads = Array.from(
          this.shadowRoot.querySelectorAll('gdq-lowerthird-name-input'));
      ...
      lowerthirdShowing.on('change', (newVal) => {
        ...
        if (newVal) {
          if (lowerthirdTimeRemaining.value === 0) {
      ...
      questionSortMap.on('change', (newVal, oldVal, operations) => {
        // If the new sortMap is equal to the currently rendered sort order,
        // do nothing.
      ...
      questionShowing.on('change', (newVal) => {
        ...
        if (newVal) {
          if (questionTimeRemaining.value === 0) {
            ...
            this.$.autoQuestion.innerText = questionTimeRemaining.value;
          }
        } else {
          this.$.autoQuestion.innerText = 'Auto';
        }
    ...
    autoQuestion() {
      ...
      nodecg.sendMessage(
        'pulseInterviewQuestion',
        questionSortMap.value[0],
        (error) => {
    ...
    showNextQuestion() {
      ...
      this._markingTopQuestionAsDone = true;
      nodecg.sendMessage(
        'interview:markQuestionAsDone', questionSortMap.value[0],
         (error) => {
           this._markingTopQuestionAsDone = false;
            if (error) {
              this.$.errorToast.text =
                'Failed to load next interview question.';
              this.$.errorToast.show();
              nodecg.log.error(error);
            }
          });
    ...
    reject(event) {
      ...
      nodecg.sendMessage(
        'interview:markQuestionAsDone', event.model.reply.id_str,
        (error) => {
    ...
    _flashBgIfAppropriate(operations) {
      if (operations && operations.length === 1) {
        ...
        // Don't flash if the change was just caused by hitting "Show Next"
        // on tier2.
        if (operations[0].method === 'splice'
              && operations[0].args.length === 2
              && operations[0].args[0] === 0
              && operations[0].args[1] === 1) {
          return;
        }
      }
    ...
    _handleDragEnd() {
      ...
      this.$.repeat.__instances.sort((a, b) => {
        ...
        // If neither of these replies are in the sort map, just leave them
        // where they are.
        if (aMapIndex < 0 && bMapIndex < 0) {
          return 0;
        }
    ...
    _mapSort(a, b) {
      ...
      // If neither of these replies are in the sort map, just leave them where
      // they are.
      if (aMapIndex < 0 && bMapIndex < 0) {
        return 0;
      }
    ...
    /* Disabled for now. Can't get drag sort and button sort to work
     * simultaneously.
