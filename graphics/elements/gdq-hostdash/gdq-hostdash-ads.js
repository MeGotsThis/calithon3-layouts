(function() {
  ...
  class GdqHostDashboardAds extends Polymer.MutableData(Polymer.Element) {
    ...
    completeAdBreak(event) {
      nodecg.sendMessage(
        'intermissions:completeAdBreak', event.detail.adBreakId);
    }
    ...
    _checkCover() {
      if (casparConnected.status !== 'declared'
          || streamingOBSWebsocket.status !== 'declared') {
        return;
      }
      ...
      const streamingOBSWebsocketIsConnected =
        streamingOBSWebsocket.value.status === 'connected';
      if (!this._connectedToNodeCG) {
        ...
        this.$.cover.innerHTML =
          'CasparCG and the streaming OBS are both disconnected!<br/>'
        ...
        this.$.cover.innerHTML = 'CasparCG is disconnected!<br/>'
          ...
          + '<br/><br/>Tell the producer immediately!';
      } else if (!streamingOBSWebsocketIsConnected) {
