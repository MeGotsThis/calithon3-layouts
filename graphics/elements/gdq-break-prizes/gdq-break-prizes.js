...
(function() {
   ...
   * @param {iron-image} target - The iron-image element to load the new src
   * into.
   * @param {string} src - The URL of the new image to load.
   * @return {Promise} - A promise that is resolved if the load succeeds,
   * and rejected it the load fails.
   */
  function loadImage(target, src) {
    return new Promise((resolve, reject) => {
      if (target.tagName !== 'IRON-IMAGE') {
        const msg =
          `Must provide an iron-image element, you provided a `
          + `"${target.tagName}".`;
  ...
  class GdqBreakPrizes extends Polymer.Element {
     ...
     * @param {Array} prizesArray - The parent array containing all the prizes
    ...
    showPrize(prize, index, prizesArray) {
      ...
      this.tl.to({}, 0.1, {
        onComplete() {
          // This will adopt either the known-good URL of a prize image,
          // or an empty string which will cause the fallback to show.
